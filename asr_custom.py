...
v2: Experimental API. It supports any models that implements ScorerInterface.''')
    ...
    parser.add_argument('--custom-task', type=str, default=None,
                        help='Function name for custom task.')
...
def main(args):
    ...
    if args.backend != "pytorch":
        raise NotImplementedError('Only pytorch backend is supported.')
    if args.num_spkrs == 1:
        from espnet.utils.dynamic_import import dynamic_import
        func = dynamic_import('espnet.asr.pytorch_backend.asr:' + args.custom_task)
        func(args)
    else:
