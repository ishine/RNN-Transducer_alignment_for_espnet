...
class DecoderRNNT(torch.nn.Module):
    ...
    def forward(self, hs_pad, hlens, ys_pad):
        ...
        """
        z, y, z_len, y_len = self.decoder_forward(hs_pad, hlens, ys_pad)
        loss = to_device(self, self.rnnt_loss(z, y, z_len, y_len))
        return loss

    def rnnt_alignment(self, hs_pad, hlens, ys_pad):
        z, y, z_len, y_len = self.decoder_forward(hs_pad, hlens, ys_pad)
        loss, alphas, betas = self.rnnt_forward_backward(z, y, z_len, y_len)
    ...
    def rnnt_forward_backward(self, acts, labels, input_lengths, label_lengths, reduction='mean'):
        log_probs = torch.nn.functional.log_softmax(acts, -1).detach().cpu().numpy()
        minibatch = log_probs.shape[0]

        loss = 0
        alphas = []
        betas = []
        for mb in six.moves.range(minibatch):
            T = input_lengths[mb].item()
            U = label_lengths[mb].item() + 1

            alpha = np.zeros((T, U), dtype=np.float64)
            beta = np.zeros((T, U), dtype=np.float64)
            blank = np.zeros((T, U), dtype=log_probs.dtype)
            label = np.zeros((T, U), dtype=log_probs.dtype)

            # init 'blank' and 'label'
            for t in six.moves.range(T):
                for u in six.moves.range(U):
                    blank[t, u] = log_probs[mb, t, u, self.blank]
                    if u < U-1:
                        label[t, u] = log_probs[mb, t, u, labels[mb, u]]
            ...
            for t in six.moves.range(T):
                for u in six.moves.range(U):
                    ...
                    if t == 0 and u > 0:
                        alpha[0, u] = alpha[0, u-1] + label[0, u-1]
                    if t > 0 and u > 0:
                        no_emit = alpha[t-1, u] + blank[t-1, u]
                        emit = alpha[t, u-1] + label[t, u-1]
                        alpha[t, u] = np.logaddexp(emit, no_emit)
            alphas.append(alpha)

            # compute 'beta'
            beta[T-1, U-1] = blank[T-1, U-1]
            for t in six.moves.range(T-1, -1, -1):
                for u in six.moves.range(U-1, -1, -1):
                    if u == U-1 and t < T-1:
                        beta[t, U-1] = beta[t+1, U-1] + blank[t, U-1]
                    if t == T-1 and u < U-1:
                        beta[T-1, u] = beta[T-1, u+1] + label[T-1, u]
                    if t < T-1 and u < U-1:
                        no_emit = beta[t+1, u] + blank[t, u]
                        emit = beta[t, u+1] + label[t, u]
                        beta[t, u] = np.logaddexp(emit, no_emit)
            betas.append(beta)

            loglike = beta[0,0] # = alpha[T-1, U-1] + blank[T-1, U-1]
            loss += -loglike

        # reduction
        if reduction == 'mean':
            loss /= minibatch
        elif not reduction in ['mean', 'sum']:
            raise NotImplementedError
